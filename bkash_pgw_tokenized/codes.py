"""bKash PGW error / status code descriptions (API + display)."""

from __future__ import annotations

# From https://developer.bka.sh/docs/error-codes plus API success code 0000
BKASH_ERROR_CODES: dict[str, str] = {
    "0000": "Successful",
...
def normalize_code(code: object | None) -> str | None:
    if code is None:
        return None
    s = str(code).strip()
...
def describe_code(code: object | None) -> str:
    key = normalize_code(code)
    if not key:
        return "Unknown error"
    return BKASH_ERROR_CODES.get(key, f"Unknown code: {key}")


def is_success_status_code(code: object | None) -> bool:
    return normalize_code(code) == "0000"
